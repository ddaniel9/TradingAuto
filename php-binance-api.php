...
class API
{
    ...
    public function __construct()
    {
        ...
        switch (count($param)) {
            ...
            case 3:
                ...
                break;
            default:
                echo 'Please see valid constructors here';
        }
    }
    ...
    {
        $file = is_null($file) ? getenv("HOME") . "/prova.json" : $file;
    ...
    public function GetMinSellBysimbol(string $symbol,$quantity=1)
    {
        $c = $this->numberOfDecimals($this->exchangeInfo()['symbols'][$symbol]['filters'][2]['minQty']);
        $quantity = $this->floorDecimal($quantity, $c);
        return $quantity;
    ...
    protected function httpRequest(string $url, string $method = "GET", array $params = [], bool $signed = false)
    {
        ...
        if (isset($json['msg']) && !empty($json['msg'])) {
            if ( $url != 'v1/system/status' && $url != 'v3/systemStatus.html' && $url != 'v3/accountStatus.html') {
                // should always output error, not only on httpdebug
                // not outputing errors, hides it from users and ends up with tickets on github
                if($json['msg']=='Filter failure: MIN_NOTIONAL'){ echo 'cccccciao'.PHP_EOL;}
     ...
     * @throws \Exception
     */
    public function getCandle(string $symbol, string $interval = "5m", int $limit = null, $startTime = null, $endTime = null)
    {
        ...
        foreach($response as $value){
                $values=array();
                $values['openTime']=handleDate($value[0]);
                $values['open']=(float)$value[1];
                $values['high']=(float)$value[2];
                $values['low']=(float)$value[3];
                $values['close']=(float)$value[4];
                $values['volume']=(float)$value[5];
                $values['closeTime']=handleDate($value[6]);
                $values['quoteAssetVolume']=(float)$value[7];
                $values['numberOfTrades']=(float)$value[8];
                $values['takerBuyBaseAssetVolume']=(float)$value[9];
                $values['takerBuyQuoteAssetVolume']=(float)$value[10];
                $values['ignore']=(float)$value[11];
                $records[]=$values;
        }
        $response= $records;
